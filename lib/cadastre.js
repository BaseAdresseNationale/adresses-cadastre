...
async function getCommune(basePath, codeCommune) {
  ...
  const {parcelles, voies, routes, numerosVoie} = await loadPCIComponents(basePath, codeCommune)
  ...
  return {parcelles: parcellesIndex, rawRoutes: routes, rawVoies: voies}
}
...
async function loadPCIComponents(basePath, codeCommune) {
  ...
  const [parcelles, voies, routes, numerosVoie] = await Promise.all([
    ...
    readGeoJSONFile(join(communePath, 'raw', `pci-${codeCommune}-zoncommuni.json.gz`)),
    readGeoJSONFile(join(communePath, 'raw', `pci-${codeCommune}-tronroute.json.gz`)),
    readGeoJSONFile(join(communePath, 'raw', `pci-${codeCommune}-numvoie.json.gz`))
  ])
