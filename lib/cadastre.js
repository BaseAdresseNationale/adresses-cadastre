...
async function getCommune(basePath, codeCommune) {
  ...
  parcelles.forEach(parcelle => {
    parcellesIndex.set(parcelle.properties.id, {
      geometry: parcelle.geometry,
      centroid: centroid(parcelle).geometry,
      numerosVoie: []
    })
  })
  numerosVoie.forEach(numeroVoie => {
    ...
    get(numeroVoie, 'extraProperties.relatedParcelles', []).forEach(({id, position}) => {
      ...
      const parcelle = parcellesIndex.get(parcelleId)
      if (parcelle) {
        parcelle.numerosVoie.push({
          id: numeroVoieId,
        ...
        console.error(`Parcelle ${parcelleId} introuvable`)
      }
    })
  })
