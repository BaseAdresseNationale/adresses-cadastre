...
function reconciliate(rawVoies, fantoir) {
  ...
  const firstPassResults = rawVoies
    .filter(rawVoie => {
      ...
      if (normalized.includes('chem') && (normalized.includes('rural') || normalized.includes('communal'))) {
        return false
      }
  ...
  if (FIRST_PASS_DEBUG) {
    firstPassResults.forEach(r => {
      if (!r.result) {
        return console.error(chalk.red(`${r.label} ne matche avec aucune entrée de FANTOIR`))
      }

      const string = `${r.label} => ${last(r.result.libelle)} (${r.result.score})`
      if (r.result.score < MIN_SCORE) {
        return console.error(chalk.yellow(string))
      }

      console.error(chalk.green(string))
    })
  }
