const {chain, keyBy, last} = require('lodash')
...
function computeVoies(locaux, fantoir, cadastre) {
  ...
  voies.forEach(voie => {
    ...
    const voieFantoir = fantoir.getVoie(voie.id)

    if (voieFantoir) {
      const nomVoieFantoir = last(voieFantoir.libelle)
      voie.nomVoieFantoir = nomVoieFantoir
      voie.libelleFantoirAccentue = accentuateWithCorpus(nomVoieFantoir, corpus)
