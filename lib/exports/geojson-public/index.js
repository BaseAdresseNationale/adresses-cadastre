...
function convertAdresse(adresse) {
  return {
    ...
    position: adresse.bestPosition.type
  }
}
...
function serialize() {
  return pipeline.obj(
    through.obj((adresse, enc, cb) => {
      if (!adresse.bestPosition || adresse.numero.startsWith('X') || !adresse.codeVoie || adresse.poidsMax === 0) return cb()
      const convertedAdresse = convertAdresse(adresse)
      cb(null, feature(adresse.bestPosition.geometry, convertedAdresse, {id: convertedAdresse.id}))
    }),
    stringify()
  )
}
