...
function serialize() {
  return pipeline.obj(
    through.obj((adresse, enc, cb) => {
      if (!adresse.position || adresse.numero.startsWith('X') || !adresse.codeVoie || adresse.poidsMax === 0) return cb()
      const convertedAdresse = convertAdresse(adresse)
      cb(null, feature(adresse.position, convertedAdresse, {id: convertedAdresse.id}))
    }),
    stringify()
  )
}
