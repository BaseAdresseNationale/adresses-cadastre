...
function extract({departement, fantoirPath, pciPath}) {
  ...
  async function flush(pushable) {
    ...
    const adresses = await extractFromCommune(locaux, codeCommune, fantoir, pciPath)
    console.error(`${codeCommune} - extraction terminée`)
