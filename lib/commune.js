...
async function extractFromCommune(locaux, codeCommune, fantoirDepartemental, pciPath) {
  ...
  return chain(byId).map(locaux => {
    ...
    const compositionTotale = chain(locaux).map('composition').flatten().uniq().value()

    const idVoie = `${codeCommune}-${codeVoie}`
    const voie = indexedVoies[idVoie]
    const libelleVoieRetenu = voie.libelleRetenu

    const adresseFormatee = formatAdresse({numero, pseudoNumero, repetition, libelleVoie: libelleVoieRetenu})

    // if (poidsMax > 0) {
    //   console.log(`${adresseFormatee} | Type de position : ${result.position_type} - Catégories : ${categoriesUtiles.join(', ')}`)
    // }
    ...
    // console.error('OK')
    return feature(result.position, {
      id,
...
function buildParcellePosition(result, codesParcelles, parcellesIndex) {
  const parcelles = chain(codesParcelles)
    .map(codeParcelle => {
      ...
      console.error('Parcelle ' + codeParcelle + ' introuvable')
      return null
    })
    .compact()
    .value()
...
function findNumPositions(numeroComplet, codesParcelles, parcelles) {
  return chain(codesParcelles)
    .map(codeParcelle => {
      ...
      // console.log(`Parcelle ${codeParcelle} introuvable`)
      return []
    })
    .flatten()
    .uniqBy('id')
